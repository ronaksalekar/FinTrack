...
const sanitizePeriod = (period) => (PERIOD_VALUES.has(period) ? period : "month");
...
const filterTransactionsByPeriod = (transactions = [], period, now = new Date()) => {
...
const buildIncomeExpenseSeries = (transactions = [], period, now = new Date()) => {
  const safePeriod = sanitizePeriod(period);
...
const getCategoryBreakdown = (transactions = [], maxItems = 6) => {
...
self.onmessage = (event) => {
  const { transactions = [], period = "month" } = event.data || {};
  const safePeriod = sanitizePeriod(period);
  const filteredTransactions = filterTransactionsByPeriod(transactions, safePeriod);
  const trendData = buildIncomeExpenseSeries(transactions, safePeriod);
